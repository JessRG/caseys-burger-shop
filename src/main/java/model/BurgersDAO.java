...
public class BurgersDAO implements Burgers {
    ...
    public Burger findById(long id) {
        ...
        // ArrayList.get(index) expects 'index' to be an int, so we have to type-cast id from a long to an int
        return burgers.get((int)id - 1);
    }

    // this method will create a Burger object in our local repository of burgers, and return the
    // newly added burger's ID
    @Override
    public long createBurger(Burger burger) {
        // the ID for each burger should be 1-indexed
        // to get the ID of a newly created burger, we'll get the size of the current
        // 'burgers' list, and add 1 to it
